...
from runner.models import Case, CaseResult
from runner.serializers import CaseSerializer, CaseResultSerializer, CaseListSerializer
...
def copy_case(request, *args, **kwargs):
    ...
    new_case = {
        "desc": case.desc + "--复制",
        "code": case.code,
        "creatorNickname": case.creator_nickname,
        "projectId": case.project_id
    }
    serializer = CaseSerializer(data=new_case)
    serializer.is_valid(raise_exception=True)
    serializer.save()
    return Response({"msg": "用例复制成功","code":0}, status=status.HTTP_200_OK)